...
- 用户标注「(Ed.) 前言」→ 新建论文集的标识，触发建立论文集文件夹，前言内容作为 `Author(Ed.)_Year_Publisher_书名_overview.md` 主体
...
books/
  Author(Ed.)_Year_Publisher/     ← 论文集文件夹
    ...
    Ch3_ChapterAuthor_Year.md                      ← 该章 sources 记录（与 PDF 同名，放在同一文件夹）
    Ch7_ChapterAuthor_Year.pdf
    Ch7_ChapterAuthor_Year.md
```

论文集每章独立处理，各自有：
- `books/Author(Ed.)_Year_Publisher/Ch3_ChapterAuthor_Year.md` + `.pdf` ← 该章 sources 记录与 PDF
- `wiki/arguments/Argument_ChapterAuthor_Year_关键词.md` ← 该章论证框架

`Author(Ed.)_Year_Publisher_书名_overview.md` 结构（与 sources/ 格式一致，放在 books/ 文件夹内）：
```markdown
---
...
4. 读取 wiki/index.md，按论文流程提取前言内容，写入对应 wiki/ 条目
5. 在 wiki/arguments/ 新建整本论文集的论证框架：
   读取 wiki/templates/template-argument-edited-volume.md
   新建 Argument_Editor_Year_Publisher.md
   （前言代表整本论文集的整体论证框架，使用论文集专用模板，不使用普通 Argument 模板）
6. 新建 books/Author(Ed.)_Year_Publisher/Author(Ed.)_Year_Publisher_书名_overview.md：
...
4. 根据文件内容判断章节标题和编号
5. 读取 wiki/index.md，按完整论文流程处理该章节（提取所有 wiki 条目）
6. 在 books/Author(Ed.)_Year_Publisher/ 新建该章 sources 记录：
   文件名：Ch3_ChapterAuthor_Year.md（与 PDF 同名）
...
8. 用 str_replace 更新 Author(Ed.)_Year_Publisher_书名_overview.md 的「已处理章节」列表，加入 Argument 链接
9. 执行双向链接维护
```
...
4. 读取 wiki/index.md，将条目名单分为待更新和待新建两组，标注类型
5. 按 vault-schema.md 工作流步骤 8、9 执行：
   - 步骤 8：逐条读取已有条目，检查格式与重构需求，整合新内容
   - 步骤 9：按类型逐条读取模板新建条目，写入 wiki/类型/ 正式文件夹
   注意：chXX.md 是章节摘要，不是 wiki 提取的替代；wiki 条目必须完整写入 wiki/ 对应文件夹
6. 新建 books/BOOKFOLDER/chXX_章节名.md，记录本章提取摘要（供全书完成后建 Argument 使用）：
```markdown
---
chapter: "chXX — 章节标题"
...
- 引用内容。（p.X）
```
注意：专著不按章节建 Argument，全书完成后读取所有 chXX.md 统一建一个整体 Argument（使用 template-argument-monograph.md）
8. 执行 vault-schema.md 工作流步骤 10（双向链接维护）
9. 停下，询问用户：「第 XX 章《章节名》处理完成，是否继续处理第 XX+1 章《章节名》？」
```
...
2. 综合所有 chXX_章节名.md，生成整本书的 sources 记录和 Argument：
   ...
   读取 wiki/templates/template-argument-monograph.md
   - 综合所有 chXX_章节名.md 的要点，填入研究问题、理论框架、论证结构、主要发现
   - 「各章概览」章节按章节顺序逐条列出每章核心贡献
   - 代表全书整体论证，不按章节拆分
