...
专著使用 git commit 追踪进度，不建立 _progress.md：
- 每章处理完后立即 commit：`git commit -m "专著名 ch01 章节名 done"`
- Session 重开后运行 `git log --oneline` 查看已处理章节
- 各章 chXX.md 列出本章提取的条目，辅助了解处理内容
...
8. Git commit：`git commit -m "书名 chXX 章节名 done"`
...
```
1. 运行 `git log --oneline` 确认所有章节均已 commit
...
6. Git commit：`git commit -m "书名 完成"`
```
