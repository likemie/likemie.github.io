...
Visible Learning 是 [[John Hattie]] 于 2009 年出版的 *Visible Learning: A synthesis of over 800 meta-analyses relating to achievement* 中提出的教育证据综合项目，将 800 余项[[Meta-analysis|元分析]]（涵盖数万项原始研究）汇总为一份"什么有效"的排名清单，以[[Effect Size|效应量]]（effect size）为标准对 150+ 项教学干预进行排序（McKnight & Whitburn, 2020, p.32）。
...
2009 年 Hattie 出版 VL 后，Cognition Education 于 2010 年推出 VLplus 品牌进行全球商业化运营，产品体系发展为三系列 16 工作坊加学校认证，通过六区域伙伴网络交付。2018 年 Corwin（SAGE 集团）收购 VLplus。完整商业化轨迹、商标时间线、产品体系、Hattie 第三方付款（2010–2014 合计 ~$869K NZD）和 Cognition Education 财务数据，详见 [[Visible Learning Branding and Global Influences]]。
...
截至 2017 年 VL 已在至少 23 个国家被采用。新西兰和澳大利亚的政策重点从外部因素转向师生互动质量（Sarbiewska, 2019）；丹麦 2014 年公立学校改革明确引用 VL，随后引发激烈反弹（Qvortrup, 2019）；俄罗斯 2017 年翻译出版 VL 俄文版，但 Golovchin（2019）建议俄教育政策不予采用。各国政策采纳的详细路径、制度嵌入和争议，详见 [[Visible Learning Branding and Global Influences]]。
...
- Knudsen（2017）发现教师和学者普遍对表达对 Visible Learning 的怀疑感到犹豫，服从是霸权性的且被视为专业性的标志（McKnight & Whitburn, 2020, p.34）
- 已有方法论批评聚焦于元分析的数学、过程或效度（Snook et al., 2009; Terhart, 2011; Wecker, Vogel, & Hetmanek, 2016），但对文化政治维度的批评相对缺乏（McKnight & Whitburn, 2020, p.32-33）
...
从统计学角度看，Hattie 以[[Effect Size|效应量]]给教学干预排序的做法要求若干技术前提成立：d 值必须可比较，效应量估计需要伴随误差信息，且边际效应不能被误读为控制第三变量后的稳健效应（Allerup, 2015, p.42）。

**d=0.40 与显著性并非固定对应。** d 本身不能直接接受常规显著性判断；只有在乘以观测数平方根、形成 `t=d√n` 后，才接近检验两个均值是否相等的 t 统计量。以 n=25 为例，双侧检验临界值 t=2.060 对应 d≈0.412，因此 Hattie 的 0.40 阈值在这一具体样本量下接近 p=0.05；但样本量变化后，这种对应关系也会改变（Allerup, 2015, pp.45–46）。这意味着 VL 的“期望效应区”并不是一个可以脱离研究规模直接使用的教育判断边界。
...
- **Cohen's d** = (M_E - M_C) / SD_pooled
- **Hedges' g** ≈ d × [1 - 3/(4N-9)]，对 d 的小样本偏差校正
- **Glass' Δ** = (M_E - M_C) / SD_control（仅用控制组标准差）
- **Fisher's z** = 0.5 × ln[(1+r)/(1-r)]，用于相关系数的正态化转换
...
1. 将每个 r 转换为 Fisher's z：z = 0.5·ln((1+r)/(1-r))
2. 计算 z 值的算术平均值
3. 将平均 z 转回 r：r = (e²ᶻ - 1)/(e²ᶻ + 1)
4. 将 r 转换为 d：d = 2r/√(1-r²)
...
- 该对照组的标准差应为 SD = SE × √n = 2.74 × √107 ≈ 28.35
...
- SD_E 应从实验组数据正确计算
- SD_pooled = √[((n_E-1)·SD²_E + (n_C-1)·SD²_C) / (n_E+n_C-2)]
...
给定 SD = 0.59 和 k = 19，Hattie 的计算给出 SE = 0.59/√19 ≈ 0.135。但正确使用 Haas 元分析中 19 个效应量的汇总值，得到的 SE 应为 0.092。
...
- Hattie 使用的标准误：0.135（公式：SD/√k = 0.59/√19 = 0.135）
- 权重（1/SE²）：1/0.135² = 1/0.0182 = 54.9
- 但正确使用 Haas 元分析中的信息：SE = 0.092
- 正确权重：1/0.092² = 1/0.0085 = 118.1
- **权重被 Hattie 低估了约 53%**
...
O'Connor（2020）对 Hattie 在 VL 中处理全语言（whole language）的方法进行了详细的案例审查，为批评者关于"先验偏见驱动数据选择"的论点提供了具体的文本证据。
...
Johnson & Janzen（2023）在 *Critical Education* 发表了对 VL 的四角度系统批判，其独特贡献在于将方法论批判、Taylorism 分析、商业化追踪和科学主义批判整合为一个统一框架。
...
- Terhart（2011）从认识论（稳定性/变异性双重困境）、教学理论（Bildung 缺失）和 Foucault 政治哲学（可见性作为精致的支配）三维度论证"很明显 Hattie 事实上没有找到圣杯"（Terhart, 2011, p.436）
...
- Nielsen & Klitmøller（2021）从 Habermas 合法化危机理论出发，论证 VL 的全球流行不是科学质量的标志，而是其技术理性内核满足了现代教育系统的合法化需求——以消除学生和教师的意向性为代价
